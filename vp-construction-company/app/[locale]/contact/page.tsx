...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // Anti-spam: Check last submission time
    const lastSubmission = localStorage.getItem('lastContactSubmission');
    const now = new Date().getTime();
    const cooldown = 60000; // 60 seconds

    if (lastSubmission && now - parseInt(lastSubmission) < cooldown) {
      const timeLeft = Math.ceil((cooldown - (now - parseInt(lastSubmission))) / 1000);
      setFormState({ status: 'error', message: `Vui lòng đợi ${timeLeft} giây trước khi gửi lại.` });
      return;
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <button type="submit" disabled={formState.status === 'loading'} className="group relative mt-4 w-full sm:w-auto px-8 py-4 bg-[#D4AF37] hover:bg-[#b5952f] text-zinc-950 font-bold tracking-[0.15em] text-[13px] transition-all duration-300 overflow-hidden disabled:bg-zinc-600 disabled:cursor-not-allowed flex items-center justify-center">
